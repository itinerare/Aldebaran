            ...
            @if(($piece->images->where('is_visible', 1)->count() + $piece->literatures->where('is_visible', 1)->count()) > 1)
                <div class="image-badge badge-primary">
                    @if($piece->images->where('is_visible', 1)->count())
                        <abbr data-toggle="tooltip" title="{{ $piece->images->where('is_visible', 1)->count() }} Image{{ $piece->images->where('is_visible', 1)->count() == 1 ? '' : 's' }}">{!! $piece->images->where('is_visible', 1)->count() == 1 ? '<i class="fas fa-image"></i>' : '<i class="fas fa-images"></i>' !!} {{ $piece->images->where('is_visible', 1)->count() }}</abbr>
                    @endif
                    ...
                    @if($piece->literatures->where('is_visible', 1)->count())
                        <abbr data-toggle="tooltip" title="{{ $piece->literatures->where('is_visible', 1)->count() }} Literature{{ $piece->literatures->where('is_visible', 1)->count() == 1 ? '' : 's' }}"><i class="fas fa-book"></i> {{ $piece->literatures->where('is_visible', 1)->count() }}</abbr>
                    @endif
                </div>
            @endif
