<div class="row">
        ...
        <div class="text-right mb-4">
            @if ($type->canCommission)
                ...
                <a class="btn btn-success"
                    href="{{ url('/commissions/' . $type->category->class->slug . '/new?type=' . $type->id . (isset($source) && $source == $type->key ? '&key=' . $type->key : '')) }}">Request
                    a Commission</a>
            @else
                <p>
    ...
    @if ($type->show_examples &&
        $type->getExamples(Auth::check() ? Auth::user() : null) &&
        $type->getExamples(Auth::check() ? Auth::user() : null)->count())
        <div class="col-md-7 align-self-center borderleft my-4">
            <div class="mobile-hide d-flex justify-content-center align-content-around flex-wrap mb-4">
                @include('gallery._flex_columns', [
                    ...
                    'source' =>
                        isset($source) && $source == $type->key
                            ? '/commissions/types/' . $type->key
                            : 'commissions/' . $type->category->class->slug,
                    'split' => 2,
                ])
            </div>
            ...
            @if ($type->getExamples(Auth::check() ? Auth::user() : null, true)->count() > 4)
                <div class="text-center mt-auto">
                    <a class="btn btn-primary"
                        href="{{ url('/commissions/types/' . (isset($source) && $source == $type->key ? $type->key : $type->id) . '/gallery') }}">More
                        Examples</a>
                </div>
            @endif
        </div>
