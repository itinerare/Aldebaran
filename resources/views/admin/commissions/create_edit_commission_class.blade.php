...
{!! breadcrumbs(['Admin Panel' => 'admin', 'Commission Classes' => 'admin/data/commission-classes', ($class->id ? 'Edit' : 'Create').' Class' => $class->id ? 'admin/data/commission-classes/edit/'.$class->id : 'admin/data/commission-classes/create']) !!}
...
{!! Form::open(['url' => $class->id ? 'admin/data/commission-classes/edit/'.$class->id : 'admin/data/commission-classes/create']) !!}
...
@if($class->id)
    <h2>Custom Pages</h2>
    <p>While all classes have two set pages (Terms of Service and Info), you can specify custom pages here as well. These pages can be edited via the <a href="{{ url('admin/pages') }}">text pages admin panel</a>. The URL of each is also displayed, for convenience, as you will need to link to any pages here manually.</p>
...
$( document ).ready(function() {
    $('.delete-class-button').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('admin/data/commission-classes/delete') }}/{{ $class->id }}", 'Delete Class');
    });
