...
@section('admin-content')
    {!! breadcrumbs([
        ...
        ($type->id ? 'Edit' : 'Create') . ' Type' => $type->id
            ? 'admin/data/commission-types/edit/' . $type->id
            : 'admin/data/commission-types/create',
    ]) !!}
    ...
    <div class="form-group">
        ...
        {!! Form::select(
            'price_type',
            ['flat' => 'Flat Cost', 'range' => 'Range', 'min' => 'Minimum', 'rate' => 'Hourly Rate'],
            $type->id ? $type->data['pricing']['type'] : null,
            ['class' => 'form-control', 'id' => 'price_type', 'placeholder' => 'Select a Pricing Type', 'required'],
        ) !!}
    </div>
    ...
    <div class="card mb-3 hide" id="flatOptions">
        <div class="card-body">
            ...
            {!! Form::number(
                'flat_cost',
                $type->id && isset($type->data['pricing']['cost']) ? $type->data['pricing']['cost'] : null,
    ...
    <div class="card mb-3 hide" id="rangeOptions">
        <div class="card-body">
            ...
            <div class="d-flex">
                {!! Form::number(
                    'cost_min',
                    $type->id && isset($type->data['pricing']['range']['min']) ? $type->data['pricing']['range']['min'] : null,
                    ['class' => 'form-control', 'placeholder' => 'Enter a Minimum Cost'],
                ) !!}
                {!! Form::number(
                    'cost_max',
                    $type->id && isset($type->data['pricing']['range']['max']) ? $type->data['pricing']['range']['max'] : null,
                    ['class' => 'form-control', 'placeholder' => 'Enter a Maximum Cost'],
                ) !!}
            </div>
        </div>
    </div>
    ...
    <div class="card mb-3 hide" id="rateOptions">
        <div class="card-body">
            ...
            {!! Form::number(
                'rate',
                $type->id && isset($type->data['pricing']['cost']) ? $type->data['pricing']['cost'] : null,
                ['class' => 'form-control', 'placeholder' => 'Enter a Rate'],
            ) !!}
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md form-group">
            ...
            {!! Form::label('is_visible', 'Is Visible', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
                'If this is turned off, visitors will not be able to see this commission type. If the type is still active, they may still request it if they have a direct link to it.',
            ) !!}
        </div>
    </div>

    <div class="form-group">
        {!! Form::label('availability', 'Available Slots (Optional)') !!} {!! add_help(
            'Number of slots available for this type at once. Filled by accepted commissions, at which point no more requests will be accepted. Set to 0 to disable.',
        ) !!}
    ...
    <div class="form-group">
        {!! Form::label('tags[]', 'Associated Tags (Optional)') !!} {!! add_help(
            'You can select up to 10 tags at once. Works with these tag(s) will be used to populate the examples gallery for this commission type, if one is displayed.',
        ) !!}
    ...
    <div class="row">
        ...
        @if ($type->id)
            <div class="col-md form-group">
                ...
                {!! Form::label('regenerate_key', 'Regenerate Key', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
                    'If checked, this will regenerate the key used for direct links to this type. Will, for good or ill, break previous links.',
                ) !!}
            </div>
        @endif
    </div>
    ...
    @if ($type->id)
        <div class="form-group">
            {!! Form::label('link', 'Link') !!} {!! add_help(
                'URL to link directly to the commission type\'s information. Can be used to link when the type is active but not visible.',
            ) !!}
            ...
            <div class="col-md">
                <div class="form-group">
                    {!! Form::checkbox(
                        'include_class',
                        1,
                        isset($type->data['include']['class']) ? $type->data['include']['class'] : 0,
            ...
            <div class="col-md">
                <div class="form-group">
                    {!! Form::checkbox(
                        'include_category',
                        1,
                        isset($type->data['include']['category']) ? $type->data['include']['category'] : 0,
                        ['class' => 'form-check-input', 'data-toggle' => 'toggle'],
                    ) !!}
                    {!! Form::label('include_category', 'Include Category Form Fields', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
                        'If this is on, the form fields from this type\'s category will be included in this type\'s forms.',
                    ) !!}
                </div>
            </div>
        </div>
