...
@section('admin-content')
    {!! breadcrumbs([
        'Admin Panel' => 'admin',
        $class->name . ' Commission Queue' => 'admin/commissions/' . $class->slug . '/pending',
    ]) !!}
    ...
    @if (Settings::get('overall_' . $class->slug . '_slots') > 0)
        <p>There {{ max($count->getSlots($class), Settings::get('overall_' . $class->slug . '_slots')) == 1 ? 'is' : 'are' }}
    ...
    {!! $commissions->render() !!}
    <div class="text-center mt-4 small text-muted">{{ $commissions->total() }}
        result{{ $commissions->total() == 1 ? '' : 's' }} found.</div>
@endsection
