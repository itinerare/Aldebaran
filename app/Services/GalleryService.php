...
class GalleryService extends Service
{
    ...
    *******************************************************************************/
    ...
    private function processImage($data, $image, $reupload = false, $regen = false)
    {
        ...
        if (Config::get('itinerare.settings.gallery_arrangement') == 'columns') {
            $thumbnail->resize(Config::get('itinerare.settings.thumbnail_width'), null, function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            });
        } else {
            $thumbnail->resize(null, Config::get('itinerare.settings.thumbnail_height'), function ($constraint) {
                $constraint->aspectRatio();
                $constraint->upsize();
            });
        }
